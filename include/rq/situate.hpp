...
struct Situator final {
  ...
  template <rq::Situation SITUATION_PARAM>
  inline void situateColonExpression(rq::Expression &expression) {
    if constexpr (SITUATION_PARAM == rq::Situation::LVALUE ||
                  SITUATION_PARAM == rq::Situation::PARAMETER ||
                  SITUATION_PARAM == rq::Situation::BINDING ||
  ...
  template <rq::Situation SITUATION_PARAM>
  inline void situateDoubleColonExpression(rq::Expression &expression) {
    if constexpr (SITUATION_PARAM == rq::Situation::LVALUE ||
                  SITUATION_PARAM == rq::Situation::PARAMETER ||
                  SITUATION_PARAM == rq::Situation::BINDING ||
                  SITUATION_PARAM == rq::Situation::ENUMERATION_VALUE) {
      ...
      this->situateBinaryExpression<SITUATION_PARAM, rq::Situation::RVALUE,
                                    rq::Situation::RVALUE>(expression);
      expression.changeKeyword(rq::Keyword::_BITWISE_CAST);
    } else {
      static_assert(false, "invalid situation");
    }
  }
  template <rq::Situation SITUATION_PARAM>
  inline void situateAssignExpression(rq::Expression &expression) {
    RQ_ASSERT(expression.getKeyword() == rq::Keyword::_ASSIGN, "wrong keyword");
    unsigned branch_i = 0;
    if (!expression.getHasBranch()) {
      this->logErrorNotExactBranchCount<SITUATION_PARAM>(expression, 2);
      this->setNotOk();
  ...
  situateAssignArithmeticExpression(rq::Expression &expression,
    ...
    rq::Expression &destination_copy =
        this->getContext().copyExpression(destination);
  ...
  inline void situateExpression(rq::Expression &expression) {
    ...
    // SITUATIONAL
    case K::_PARENTHESIS:
      if constexpr (!getCanBeSituation<SP>(K::_PARENTHESIS)) {
        RQ_UNREACHABLE();
      } else {
      ...
      break;
    case K::_COLON:
      if constexpr (!getCanBeSituation<SP>(K::_COLON)) {
        RQ_UNREACHABLE();
      } else {
    ...
    case K::_DOUBLE_COLON:
      if constexpr (!getCanBeSituation<SP>(K::_DOUBLE_COLON)) {
        RQ_UNREACHABLE();
      } else {
        this->situateDoubleColonExpression<SP>(expression);
      }
      break;
    case K::_INFERENCE:
    ...
    case K::_ORDERED_BINDING:
      if constexpr (!getCanBeSituation<SP>(K::_ORDERED_BINDING)) {
        RQ_UNREACHABLE();
      } else {
        this->situateBinaryExpression<SP, S::RVALUE>(expression);
      }
    ...
    case K::_UNORDERED_BINDING:
      if constexpr (!getCanBeSituation<SP>(K::_UNORDERED_BINDING)) {
        RQ_UNREACHABLE();
      } else {
        this->situateBinaryExpression<SP, S::RVALUE>(expression);
      }
    ...
    // MEMORY
    case K::_CONCATINATE:
      if constexpr (!getCanBeSituation<SP>(K::_CONCATINATE)) {
        RQ_UNREACHABLE();
      } else {
        this->situateNaryExpression<SP, 2, S::RVALUE>(expression);
      }
    ...
    case K::NO_SHALLOW_COPY:
      if constexpr (!getCanBeSituation<SP>(K::NO_SHALLOW_COPY)) {
        RQ_UNREACHABLE();
      } else {
        this->situateNullaryExpression<SP>(expression);
      }
    ...
    case K::_STRUCTURED_BINDING:
      if constexpr (!getCanBeSituation<SP>(K::_STRUCTURED_BINDING)) {
        ...
        this->situateNaryWithLastExpression<SP, 2, S::BINDING, S::RVALUE>(
            expression);
      }
      break;
    case K::_IGNORE:
    ...
    // ATTRIBUTES
    case K::BAKE:
      if constexpr (!getCanBeSituation<SP>(K::BAKE)) {
        RQ_UNREACHABLE();
      } else {
        this->situateNullaryExpression<SP>(expression);
      }
